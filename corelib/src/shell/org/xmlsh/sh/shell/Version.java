...
public class Version {
  ...
  public enum Key implements Supplier<String> {
    RELEASE("release", Version::getRelease), VERSION("version",
        Version::getVersion), SAXON_VERSION("saxon-version",
            Version::getSaxonVersion), SAXON_EDITION("saxon-edition",
                Version::getSaxonEdition), JAVA_VERSION("java-version",
                    Version::getJavaVersion), JAVA_NAME("java-name",
                        Version::getJavaName), JAVA_HOME("java-home",
                            Version::getJavaHome), BUILD("build",
  ...
  public static enum Op implements BiFunction<String, String, Boolean> {
    GT("gt", (l, r) -> toNum(l).compareTo(toNum(r)) > 0), LT("lt",
        (l, r) -> toNum(l).compareTo(toNum(r)) < 0), EQ("eq",
            (l, r) -> toNum(l).equals(toNum(r))), GTE("gte",
                (l, r) -> toNum(l).compareTo(toNum(r)) >= 0), LTE("lte",
                    (l, r) -> toNum(l).compareTo(toNum(r)) <= 0), MATCHES(
                        "matches", (l, r) -> l.matches(r));
  ...
  static {
    ...
    try (
        InputStream stream = Version.class
            .getResourceAsStream("version.properties");) {
      assert (stream != null);
      if(stream == null)
        mLogger.throwing(new UnexpectedException(
            "Cannot locate version.properties in classpath"));
      else
        mProperties.load(stream);

    } catch (IOException e) {
      mLogger.throwing(new UnexpectedException(
          "Cannot locate version.properties in classpath", e));
    }
  }
  ...
  public static String getBuildDate() {
    return getProperty("version.build_date");
  }

  public static String getRelease() {
    return getProperty("version.release");
  }
