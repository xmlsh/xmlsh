...
public class ShellThread extends Thread implements Closeable {
  ...
  public ShellThread(ThreadGroup threadGroup, Shell shell, String name,
      ICommandExpr cmd) {
    super(threadGroup == null ? shell.getThreadGroup() : threadGroup,
        simpleName(name, cmd));
    mShell = shell;
    mCommand = cmd;
  ...
  public void run() {
    ...
    try {
      ...
      shell.setCurdir(mIniitalCD); // Populate the current directory in this
                                   // thread
      mCommand.exec(shell);
      ...
      // + e.toString() );
      mLogger.error("Exception running command: " + mCommand.describe(false),
          e);
