 ...
 * -encoding encoding Read CSV format in the specified encoding, else cp1252
 * assumed
 * -quote quoted by (default ")
 * -skip lines Skip # lines before reading header or data ||
 * -trim Trim the output by ignoring any data after the last column specified in
 * -colnames or -header
 * -max # Combine the data in the last field by ignoring any more delimiters
 * after the # of fields is specifed, or if # is <=0 then use the number of
 * colums
 */
...
public class csv2xml extends XCommand {
  ...
  public int run(List<XValue> args) throws Exception {
    ...
    Options opts = new Options(
        "nameing=naming-strategy:,root:,row:,col:,header,attr,delim:,quote:,colnames:,tab,skip:,trim,max:",
    ...
    CSVParser parser = new CSVParser(ir, delim.charAt(0), quote.charAt(0),
        maxFields);
  ...
  private void addElement(
      ...
      boolean battr,
      CSVRecord header,
      boolean bTrim) throws XMLStreamException {
  ...
  private QName getAttrName(int i, String col, CSVRecord header,
      boolean bTrim) {
    if(header != null) {
      if(header.getNumFields() <= i)
