...
public class xed extends XCommand {
  ...
  public int run(List<XValue> args)
    ...
    if(opt_matches == null && opt_xpath == null)
      throw new InvalidArgumentException(
          "option xpath or matches must be specified");
    ...
    if(opt_replace != null || opt_delete || opt_add != null
        || opt_replacex != null || opt_rename != null) {

      Iterable<XdmItem> results = getResults(eval, context,
          opt_matches != null);
  ...
  private void add(MutableNodeInfo node, XValue add)
      throws IndexOutOfBoundsException, SaxonApiUncheckedException,
      SaxonApiException, InvalidArgumentException {
    if(!add.isAtomic()) {
  ...
  private void replace(MutableNodeInfo node, XValue replace,
      boolean compat_mode)
      throws IndexOutOfBoundsException, SaxonApiUncheckedException,
      SaxonApiException, XPathException, InvalidArgumentException {
    if(!replace.isAtomic()) {
    ...
    else if(compat_mode || node.getNodeKind() == Type.ATTRIBUTE)
      node.replaceStringValue(replace.toString());
    else
      node.replace(
          new NodeInfo[] { createTextNode(node, replace.toXdmValue()) }, true);
  }

  private void rename(MutableNodeInfo node, String opt_rename)
  ...
  private void replace(XdmItem item, MutableNodeInfo node,
      XPathSelector replacex)
      throws IndexOutOfBoundsException, SaxonApiUncheckedException,
      SaxonApiException, XPathException, InvalidArgumentException {
  ...
  private NodeInfo createTextNode(MutableNodeInfo parent, XdmValue value)
