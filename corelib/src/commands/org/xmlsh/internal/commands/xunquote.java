...
public class xunquote extends XCommand {
  ...
  public int run(List<XValue> args) throws Exception {
    ...
    /*
     * IF there are any arguments, then treat them as strings and re-parse them
     * to XML
    ...
    if(args.size() > 0)
      for(XValue arg : args) {
        ...
        UnquotingInputStream in = new UnquotingInputStream(
            tempBuf.toByteArray());

        XMLInputFactory factory = XMLInputFactory.newInstance();
        XMLEventReader reader = factory.createXMLEventReader(in,
            serializeOpts.getInputXmlEncoding());
  ...
  private void copyXML(XMLEventReader in, XMLEventWriter out)
