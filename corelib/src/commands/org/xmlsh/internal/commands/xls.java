...
public class xls extends XCommand {
  ...
  public int run(List<XValue> args) throws Exception {
    Options opts = new Options("a=all,l=long,R=recurse,r=relative,s=system",
    ...
    for(XValue arg : args) {
      ...
      if(path == null || !Files.exists(path, LinkOption.NOFOLLOW_LINKS)) {
        this.printErr(
            "ls: cannot access " + sArg + " : No such file or directory");
        ret++;
        continue;
      }
   ...
   * 
   * @Override
   * public void visitFile( boolean root,Path path, BasicFileAttributes attrs)
   * throws IOException{
   * 
