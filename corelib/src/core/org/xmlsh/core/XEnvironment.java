...
public class XEnvironment {
  ...
  public XEnvironment(Shell shell, StaticContext ctx, IModule mod, ShellIO io)
      throws IOException {
  ...
  public XVariable getVar(String name) {
    ...
    case "#":
      return XVariable.newInstance(name,
          XValue.newXValue(mShell.getArgs().size()));
    case "$":
      return XVariable.newInstance(name,
          XValue.newXValue(Thread.currentThread().getId()));
    case "?":
      return XVariable.newInstance(name, XValue.newXValue(mShell.getStatus()));
    case "!":
      return XVariable.newInstance(name,
          XValue.newXValue(mShell.getLastThreadId()));
    }
  ...
  public void setIndexedVar(String name, XValue value, String ind)
      throws CoreException {
    XVariable var = mVars.get(name);
    if(var == null)
      var = XVariable.newInstance(name,
          XValue.newXValue(TypeFamily.XTYPE, new XValueMap()),
          effectiveFlags(null));
    else
      var = var.clone();
   ...
   */
  public void appendVar(String name, XValue xvalue)
  ...
  public XVariable declareVar(String name, EnumSet<XVarFlag> flags,
      XValue value) throws InvalidArgumentException {
    XVariable var = mVars.get(name);
    if(var == null)
  ...
  public XVariable setVar(String name, XValue value)
      throws InvalidArgumentException {
  ...
  public XVariable setVar(String name, XValue value,
      EnumSet<XVarFlag> explicitFlags) throws InvalidArgumentException {
  ...
  public XEnvironment clone(Shell shell) throws IOException {
    ...
    mLogger.entry(shell);
    XEnvironment that = new XEnvironment(shell, getStaticContext().clone(),
        getModule(), null);
  ...
  public OutputStream getOutputStream(String file, boolean append,
      SerializeOpts opts) throws FileNotFoundException,
  ...
  public boolean isStdinSystem() {
    return getStdin().isSystem();
  }

  public boolean isStdoutSystem() {
    return getStdout().isSystem();
  }

  public boolean isStderrSystem() {
    return getStderr().isSystem();
  }
   ...
   */
  public OutputPort getOutput(XValue port, boolean append)
      throws IOException, InvalidArgumentException {

    mLogger.entry(port,append);
    if(port == null)
      return getStdout();
    if(port.isAtomic()) {
      ...
      mLogger.trace("creating new output port {}",name);
  ...
  public OutputPort getOutput(String port, boolean append)
      throws IOException, InvalidArgumentException {
  ...
  public void setStdin(XVariable variable)
      throws IOException, InvalidArgumentException {
  ...
  public InputPort setInput(String name, XVariable variable)
      throws IOException, InvalidArgumentException {
  ...
  public InputStream getInputStream(XValue file, SerializeOpts opts)
      throws IOException, InvalidArgumentException, CoreException {
    return getInput(file).asInputStream(opts);
  }

  public Source getSource(XValue value, SerializeOpts opts)
      throws IOException, InvalidArgumentException, CoreException {
   ...
   * if looks like "scheme://path" return a port based on an input stream from
   * UI
   ...
   * 
   */
  public InputPort getInput(XValue port)
      throws IOException, InvalidArgumentException {
  ...
  public InputSource getInputSource(XValue value, SerializeOpts opts)
  ...
  public IModule getModuleByPrefix(String prefix) {
    ...
    return mLogger.exit(
        getStaticContext().getModules().getExistingModuleByPrefix(prefix));
  ...
  public Collection<String> getPrefixesForModule(IModule hm) {
    ...
    return mLogger
        .exit(getStaticContext().getModules().getPrefixesForModule(hm));
  ...
  public void initVariable(XVariable var) {
    ...
    if(mVars.containsKey(var.getName())) {
      mLogger.debug("Attempt to initialize a pre-existing variable: {}",
          var.getName());
    }
