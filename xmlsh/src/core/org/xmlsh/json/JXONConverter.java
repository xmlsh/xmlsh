...
public class JXONConverter extends JXConverter
{
	...
	class JConverter extends JSONConverter
	{
		...
		private boolean writeFile(StartElement start) throws UnsupportedEncodingException, IOException,
		                                             XMLStreamException, CoreException, TransformException,
		                                             SaxonApiException
		{
			...
			// Attribute aencoding = start.getAttributeByName(new
			// QName("encoding"));
			// String encoding = (aencoding == null ? "UTF-8" :
			// aencoding.getValue());

			/*
			 * PrintWriter w = getShell().getEnv().getOutput(
			 * getShell().getFile(name),
			 * false).asPrintWriter(getSerializeOpts());
			 * 
			 * boolean ret = parse(reader,w); w.close(); return ret ;
			 */

			mLogger.warn(kELEM_FILE + "Not Supported");
			return false;
	...
	class XConverter extends XMLConverter
	{
		...
		@Override
		void writeNumber(long v) throws ConverterException
		{
			try {
				writeStartElement(kELEM_NUMBER);
				writeCharacters(Long.toString(v));
				writeEndElement();
			} catch (XMLStreamException e) {
				throw new ConverterException(e);
			}
		...
		@Override
		void writeNumber(double v) throws ConverterException
		{
			try {
				writeStartElement(kELEM_NUMBER);
				writeCharacters(Double.toString(v));
				writeEndElement();
			} catch (XMLStreamException e) {
				throw new ConverterException(e);
			}
