...
public class xconv extends XCommand {
    ...
    private Consumer<? super String> linewriter(final String eol,
            PrintWriter out) {
        return (l) -> { out.print(l) ; if( eol != null ) out.print(eol); };
    }
	...
	public int run(List<XValue> args) throws Exception {
		...
		Options opts = new Options( "eol-mode:,dos2unix=d2u,unix2dos=u2d,crlf=dos,lf=unix,system,java,nul" , SerializeOpts.getOptionDefs()  );
		opts.parse(args);
		...
		try (
		   PrintWriter out = getStdout().asPrintWriter(getSerializeOpts(opts));
            ...
            if( args.size() > 0 ){
        			for( XValue arg : args ){
        			    try (
        				   BufferedReader in = new BufferedReader(getShell().getEnv().getInput(arg).asReader(getSerializeOpts()));
        			    ) {				
        		    ...
        		    try (
        	                   BufferedReader in = new BufferedReader(getShell().getEnv().getStdin().asReader(getSerializeOpts()));
        	                ) {             
                              in.lines().forEach(  eolwriter );
         	            }
