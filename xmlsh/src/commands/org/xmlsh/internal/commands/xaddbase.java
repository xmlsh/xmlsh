...
public class xaddbase extends XCommand {
  ...
  private static QName mBaseQName = new QName(
      "http://www.w3.org/XML/1998/namespace", "base", "xml");
  ...
  public int run(List<XValue> args)
    ...
    Options opts = new Options("a=all,r=relative",
  ...
  private XMLEvent add_base_attr(StartElement start, String base) {
    ...
    return XMLStreamUtils.mergeAttributes(start,
        Collections.singletonList(attr).iterator(), mFactory);

  }

  private String resolve(String parentBaseURI, String baseURI,
      boolean opt_relative) throws URISyntaxException {
    if(!opt_relative)
      return baseURI;
    ...
    URI parent_URI = new URI(u.getScheme(),
  ...
  private void add_base(XMLEventReader reader, XMLEventWriter writer,
      boolean opt_all, boolean opt_relative, String parentURI)
    ...
    while(reader.hasNext()) {
      ...
      if(event.getEventType() == XMLStreamConstants.START_ELEMENT) {
        ...
        if(bIsRoot) { // document root
          ...
          addBase = true;
        else if(baseURI.equals(parentURI))
          addBase = false;
        else if(!Util.isEqual(parentURI, baseURI))
          addBase = true;

        if(addBase) {
          String uri = resolve(parentURI, baseURI,
              bIsRoot ? false : opt_relative);
