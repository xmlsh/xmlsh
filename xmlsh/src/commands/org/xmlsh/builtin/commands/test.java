...
public class test extends BuiltinCommand {
	...
	private boolean eval(List<XValue> av) throws InvalidArgumentException,
			UnexpectedException, Error, IOException {
		if (av.size() == 0)
			return false;
		...
		if (av1.isAtomic()) {
			...
			if (a1.equals("(")) {
				...
				if (pdepth > 0
						&& (av.get(0).isAtomic() && av.get(0).toString()
								.equals(")"))) {
					av.remove(0);
					pdepth--;
			...
			} else if (a1.startsWith("-") && !Util.isInt(a1, true)) {
				if (av.size() < 1) {
	...
	private int compareInt(XValue a1, XValue a2) throws Error {
		...
		return Util.parseInt(s1, 0) - Util.parseInt(s2, 0);

	}

	private boolean evalUnary(String op, XValue value)
			throws InvalidArgumentException, UnexpectedException, IOException,
			Error {
	...
	private boolean isURI(XValue value) {
		return value.toString().matches("^[a-z]+://.*");
	}
	...
	public int run(List<XValue> args) throws Exception {
		...
		try {
			while (av.size() > 0) {
				String op = null;
				XValue a1 = av.get(0);
				if (!bFirst && a1.isAtomic()
						&& (a1.equals("-a") || a1.equals("-o"))) {
					op = a1.toString();
					av.remove(0);
				}
