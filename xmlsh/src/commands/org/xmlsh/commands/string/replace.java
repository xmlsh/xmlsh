...
public class replace extends BuiltinFunctionCommand {
    ...
    public XValue run(Shell shell, List<XValue> args) throws UnexpectedException, XPathException,
        ...
        if (args.size() < 3)
            throw new InvalidArgumentException("usage: replace(string search replace");
        String str = args.get(0).toString();
        String search = args.get(1).toString();
        String replace = args.get(2).toString();

        return new XValue(str.replaceAll(search, replace));
    }
