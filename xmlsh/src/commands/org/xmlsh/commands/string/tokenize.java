...
public class tokenize extends BuiltinFunctionCommand {
    ...
    public XValue run(Shell shell, List<XValue> args) throws UnexpectedException, XPathException,
        ...
        if (args.size() < 2)
            throw new InvalidArgumentException("usage: tokenize(string [expr])");
        String str = args.get(0).toString();
        String tok = args.size() > 0 ? args.get(1).toString() : "[ \t\r\n]";
        int limit = args.size() > 1 ? args.get(2).toInt() : 0;
        return new XValue(str.split(tok, limit));
    }
