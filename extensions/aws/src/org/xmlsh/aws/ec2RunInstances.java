...
public class ec2RunInstances extends AWSEC2Command {
    ...
    private int runInstance( String ami_id, Options opts ) throws IOException, XMLStreamException, SaxonApiException, CoreException 
    {
 	...
 	ec2-run-instances ami_id [-n instance_count] [-g group [-g group ...]] 
        ...
        if( opts.hasOpt("ebs-optimized"))
          request.setEbsOptimized( true);
        ...
        if( result != null && opts.hasOpt("tag" ) )
            setTags( result.getReservation() , opts.getOptValues("tag") );
        writeResult( result );

        return 0;
    }

    private int setTags(Reservation reservation, List<XValue> tags ) throws InvalidArgumentException, IOException, XMLStreamException, SaxonApiException {
        ArrayList<String> list = new ArrayList<String>();
        for( Instance i : reservation.getInstances() )
            list.add(i.getInstanceId());
        int ret = createTags( parseTags( tags )  , list  );
        return ret;
    }
