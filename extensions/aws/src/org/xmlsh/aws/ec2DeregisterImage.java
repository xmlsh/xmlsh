...
public class ec2DeregisterImage extends AWSEC2Command {
    ...
    public int run(List<XValue> args) throws Exception {
        ...
        int ret = deregisterImage( args.get(0).toString(), opts.getOptFlag("delete-snapshot", false));

        return ret;	
    }

    private int deregisterImage( String image, boolean deleteSnap ) throws IOException, XMLStreamException, SaxonApiException, CoreException 
    {

        mLogger.entry(image, deleteSnap);
        
        String snapshotID = null ;
        DescribeImagesRequest request = new DescribeImagesRequest().withImageIds(image);
        if( request != null ){
            ...
            if( result != null){
                if( result.getImages().size() == 1 ){    
                    ...
                    mLogger.debug("Found image root device: %1 type %2" , rootName , rootType );
        ...
        if( deleteSnap && !Util.isBlank(snapshotID) ){
            ...
            try {
             getAWSClient().deleteSnapshot(new DeleteSnapshotRequest().withSnapshotId(snapshotID));
    ...
    private void writeResult(String image, String snapshotID, boolean deleted) throws IOException, XMLStreamException, SaxonApiException, CoreException {
