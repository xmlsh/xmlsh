...
public class s3cp extends AWSS3Command {
  ...
  private int copy(S3Path src, S3Path dest, String storage,
      boolean onlyIfExists, boolean copyAcl, boolean copyMetadata) {
    ...
    ObjectMetadata meta = copyMetadata
        ? client.getObjectMetadata(src.getBucket(), src.getKey()) : null;
    AccessControlList acl = copyAcl
        ? client.getObjectAcl(src.getBucket(), src.getKey()) : null;

    traceCall("copyObject");

    CopyObjectRequest request = new CopyObjectRequest(src.getBucket(),
        src.getKey(),
