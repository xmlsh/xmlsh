...
public class glacierPutArchive extends AWSGlacierCommand {
  ...
  private int put(String vault, String desc, List<XValue> files, Options opts)
      throws IOException, XMLStreamException, SaxonApiException, CoreException {

    OutputPort stdout = this.getStdout();
    mWriter = stdout.asXMLStreamWriter(getSerializeOpts());

    ArchiveTransferManager tm = new ArchiveTransferManager(getAWSClient(),
        new AWSCommandCredentialsProviderChain(getShell(), opts));
