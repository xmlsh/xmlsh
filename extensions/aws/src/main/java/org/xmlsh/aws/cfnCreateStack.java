...
public class cfnCreateStack extends AWSCFNCommand {
  ...
  public int run(List<XValue> args) throws Exception {
    ...
    Options opts = getOptions(
        "capability:+,disable-rollback,fail=on-failure:,notification-arn:+,name:,template-file=f:,template-url=url:,timeout:,tag:+,params=parameters:");
  ...
  private int createStack(Options opts)
    ...
    if(opts.hasOpt("notification-arn"))
      request.setNotificationARNs(
          Util.toStringList(opts.getOptValues("notification-arn")));
   ...
   */
  private Collection<Parameter> getParameters(Options opts)
  ...
  private Collection<? extends Parameter> getParameters(XValue x)
      throws InvalidArgumentException {
  ...
  private void writeStackResult(CreateStackResult result, String name)
