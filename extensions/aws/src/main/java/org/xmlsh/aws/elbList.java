...
public class elbList extends AWSELBCommand {
  ...
  private int list(List<String> elbs)
    ...
    DescribeLoadBalancersRequest request = elbs.size() == 0
    ...
    DescribeLoadBalancersResult result = getAWSClient()
    ...
    for(LoadBalancerDescription desc : result.getLoadBalancerDescriptions()) {
      ...
      DescribeInstanceHealthRequest healthRequest = new DescribeInstanceHealthRequest(
          desc.getLoadBalancerName());
      DescribeInstanceHealthResult healthResult = getAWSClient()
          .describeInstanceHealth(healthRequest);
      ...
      attribute("unhealthy-threshold",
          health.getUnhealthyThreshold().toString());
      endElement(); // health
      ...
      for(ListenerDescription listen : desc.getListenerDescriptions()) {
        ...
        attribute("ssl-certificate-id",
            listen.getListener().getSSLCertificateId());
        attribute("instance-port",
            listen.getListener().getInstancePort().toString());
        attribute("load-balancer-port",
            listen.getListener().getLoadBalancerPort().toString());
