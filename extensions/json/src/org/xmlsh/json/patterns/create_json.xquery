...
declare function local:tojson_attribute($e as element(jxon:attribute), $pattern as element(jxon:pattern))
{
  ...
  return
    (common:dump($e, $pattern), $common:nl, 
...
declare function local:tojson_element_contents($e as element(jxon:element), $pattern as element(jxon:pattern))
{
    ...
    <array>
      {
        ...
        else <xsl:apply-templates select="*" mode="wrap"/>
      }
...
declare function local:tojson_element($e as element(jxon:element), $pattern as element(jxon:pattern))
{
  ...
  return
    (common:dump($e, $pattern), $common:nl, 
     ...
     {
		let $contents := local:tojson_element_contents( $e , $pattern)
		return
...
{
  for $e in $es
  let $pattern := common:getpattern($e)
  return
    (typeswitch ($e)
     case $elem as element(jxon:element) return local:tojson_element($elem, $pattern)
     case $a as element(jxon:attribute) return local:tojson_attribute($a, $pattern)
     case $d as element(jxon:document) return local:tojson_document($d, $pattern)
     default return (), 
           ...
           <xsl:stylesheet version="2.0" xmlns="http://www.xmlsh.org/jxml">
             {
               ...
               return attribute { fn:QName($ns/@uri, concat($ns/@prefix, ":bogus")) } {}
             }
...
<metaInformation>
	<scenarios>
		...
		<scenario default="no" name="docbook" userelativepaths="yes" externalpreview="no" useresolver="yes" url="" outputurl="" processortype="saxon" tcpport="0" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline=""
		          additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" host="" port="0" user="" password="" validateoutput="no" validator="internal"
		          customvalidator="">
			<parameterValue name="{http://www.xmlsh.org/jsonxml/common}patterns" value="doc('patterns.xml')"/>
